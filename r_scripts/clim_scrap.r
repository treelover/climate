...
for(i in 1:length(slist)){
	...
	for(j in 1:length(dayfac)){
		...
		daydat = rbind(daydat,as.numeric(c(substr(dayfac[j],1,2),colMeans(tab,na.rm=T),nyear)))	
	...
	colnames(daydat) = c('Month',colnames(tab),'nyear')
...
#Writing her out!
WriteXLS(x = 'daylist',ExcelFileName = paste(destdir,'Monthly_means.xls',sep=''),SheetNames = names(daylist),AdjWidth = T)
