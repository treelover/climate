...
#---------------------------------------#
#First the "extended" ones
load('/home/adam/Dropbox/sapflow/veg/mam/mam_climdata.Rdat')
mama = final

load('/home/adam/Dropbox/sapflow/veg/lau/lau_climdata.Rdat')
lau = final

load('/home/adam/Dropbox/sapflow/veg/palam/palam_climdata.Rdat')
palam = final

load('/home/adam/Dropbox/sapflow/veg/pww/pww_climdata.Rdat')
pww = final
...
slist = list(lau,mama,palam,pww)
names(slist) = c('lau','mama','palam','pww')
#---------------------------------------#
...
for(i in 1:length(slist)){
	...
	#Make an index of each month...named dayfac because this code is a straight port of the daily summary script. 
	dayfac = sort(unique(substr(dat$yyyy.mm.dd.hh.mm,1,4)))
	...
	for(j in 1:length(dayfac)){
		...
		dID = which(substr(dat$yyyy.mm.dd.hh.mm,1,4) == dayfac[j])
		tempdat = dat[dID,]
		...
		daydat = rbind(daydat,as.numeric(c(substr(dayfac[j],1,4),colMeans(tab,na.rm=T),nyear)))	
	...
	colnames(daydat) = c('Year',colnames(tab),'nyear')
...
#Writing her out!
WriteXLS(x = 'daylist',ExcelFileName = paste(destdir,'Yearly_means.xls',sep=''),SheetNames = names(daylist),AdjWidth = T)
