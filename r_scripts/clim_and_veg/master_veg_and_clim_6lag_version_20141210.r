...
#Taking out the few entries in veg13 that are in 2014
veg13$DATE_2[which(substr(veg13$DATE_2,1,4) == '2014')] = NA
...
for(i in 1:nrow(veg)){
	...
	win2010 = which(clim$yyyy.mm.dd.hh.mm == (as.POSIXct(paste(veg$survey_2010[i],'00:00:00'))-lag))
	win2010 = which(clim$yyyy.mm.dd.hh.mm == as.POSIXct(paste(veg$survey_2010[i],'00:00:00'))-(per+lag)):win2010
	
	win2011 = which(clim$yyyy.mm.dd.hh.mm == (as.POSIXct(paste(veg$survey_2011[i],'00:00:00'))-lag))
	win2011 = which(clim$yyyy.mm.dd.hh.mm == as.POSIXct(paste(veg$survey_2011[i],'00:00:00'))-(per+lag)):win2011
	
	win2012 = which(clim$yyyy.mm.dd.hh.mm == (as.POSIXct(paste(veg$survey_2012[i],'00:00:00'))-lag))
	win2012 = which(clim$yyyy.mm.dd.hh.mm == as.POSIXct(paste(veg$survey_2012[i],'00:00:00'))-(per+lag)):win2012
	
	win2013 = which(clim$yyyy.mm.dd.hh.mm == (as.POSIXct(paste(veg$survey_2013[i],'00:00:00'))-lag))
	win2013 = which(clim$yyyy.mm.dd.hh.mm == as.POSIXct(paste(veg$survey_2013[i],'00:00:00'))-(per+lag)):win2013
...
WriteXLS(x = 'laumasta',ExcelFileName = paste(vdir,'Lau_master_climveg_6mo_6lag_version_20141210.xls',sep=''),AdjWidth = T)
...
#write it out for its own self
WriteXLS(x = 'palmasta',ExcelFileName = paste(vdir,'Pal_master_climveg_6mo_6lag_version_20141210.xls',sep=''),AdjWidth = T)
...
for(i in 1:nrow(veg)){
	...
	#Coming up with the aggregation window for each year	
	win2012 = which(clim$yyyy.mm.dd.hh.mm == (as.POSIXct(paste(veg$survey_2012[i],'00:00:00'))-lag))
	win2012 = which(clim$yyyy.mm.dd.hh.mm == as.POSIXct('2011-03-16 00:00:00')):win2012
	
	win2013 = which(clim$yyyy.mm.dd.hh.mm == (as.POSIXct(paste(veg$survey_2013[i],'00:00:00'))-lag))
	win2013 = which(clim$yyyy.mm.dd.hh.mm == as.POSIXct(paste(veg$survey_2013[i],'00:00:00'))-(per+lag)):win2013
	
	win2014 = which(clim$yyyy.mm.dd.hh.mm == (as.POSIXct(paste(veg$survey_2014[i],'00:00:00'))-lag))
	win2014 = which(clim$yyyy.mm.dd.hh.mm == as.POSIXct(paste(veg$survey_2014[i],'00:00:00'))-(per+lag)):win2014
...
#write it out for its own self
WriteXLS(x = 'mamamasta',ExcelFileName = paste(vdir,'Mam_master_climveg_6mo_6lag_version_20141210.xls',sep=''),AdjWidth = T)

#Fix the masta survey_2014 column 
masta$survey_2014 = as.POSIXlt(masta$survey_2014)	

masta = rbind(masta,mamamasta)
WriteXLS(x = 'masta',ExcelFileName = paste('/home/adam/Dropbox/sapflow/veg/master_climveg_6mo_6lag_version_20141210.xls',sep=''),AdjWidth = T)
#============================================================#
#------------------------------------------------------------#
#............................................................#
#------------------------------------------------------------#
...
id12 = which(substr(test[,1],1,4) == '2012')
id13 = which(substr(test[,1],1,4) == '2013')
id14 = which(substr(test[,1],1,4) == '2014')

rf12 = mat.or.vec(length(id12),nc=1)
rf13 = mat.or.vec(length(id13),nc=1)
rf14 = mat.or.vec(length(id14),nc=1)

rf12[1] = test[id12[1],'RF']
rf13[1] = test[id13[1],'RF']
rf14[1] = test[id14[1],'RF']

RF = test[,'RF']
RF[which(is.na(RF))] = 0


for(i in 2:length(rf12)){rf12[i] = RF[id12[i]]+rf12[i-1]}
for(i in 2:length(rf13)){rf13[i] = RF[id13[i]]+rf13[i-1]}
for(i in 2:length(rf14)){rf14[i] = RF[id14[i]]+rf14[i-1]}

RFp = c(rf12,rf13,rf14)

plot(y = RFp,x = test[,1],xlab = 'time',ylab = 'Cumulative RF, by year (mm)',type='l',col = 'blue',lwd = 3,
main = 'Cumulative Rainfall at PWW by year. Record starts at Oct 16th, 2012')
savePlot(filename = '/home/adam/Dropbox/sapflow/veg/pww/rainfall.jpeg',type='jpeg')
