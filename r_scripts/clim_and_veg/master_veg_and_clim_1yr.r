...
WriteXLS(x = 'laumasta',ExcelFileName = paste(vdir,'Lau_master_climveg_12mo.xls',sep=''),AdjWidth = T)
...
#write it out for its own self
WriteXLS(x = 'palmasta',ExcelFileName = paste(vdir,'Pal_master_climveg_12mo.xls',sep=''),AdjWidth = T)
	
masta = rbind(laumasta,palmasta)
WriteXLS(x = 'masta',ExcelFileName = paste('/home/adam/Dropbox/sapflow/veg/master_climveg_12mo.xls',sep=''),AdjWidth = T)
#============================================================#
#------------------------------------------------------------#
#............................................................#
#------------------------------------------------------------#
...
for(i in 1:nrow(veg)){
	...
	#Coming up with the aggregation window for each year
	win2012 = which(clim$yyyy.mm.dd.hh.mm == as.POSIXct(paste(veg$survey_2012[i],'00:00:00')))
	win2012 = which(clim$yyyy.mm.dd.hh.mm == as.POSIXct('2012-03-16 00:00:00')):win2012
	...
	#2012
	if(length(which(is.na(Tair[win2012])))/length(win2012) < nathresh){
	vegclim$Tmin_2012[i] = mean(daymins[which(dayuni == as.POSIXct('2012-03-16 00:00:00')):which(dayuni == veg$survey_2012[i])],na.rm=T)
	vegclim$Tmax_2012[i] = mean(daymaxs[which(dayuni == as.POSIXct('2012-03-16 00:00:00')):which(dayuni == veg$survey_2012[i])],na.rm=T)}
...
#write it out for its own self
WriteXLS(x = 'mamamasta',ExcelFileName = paste(vdir,'Mam_master_climveg_12mo.xls',sep=''),AdjWidth = T)
