...
for(i in 1:length(slist)){
	...
	for(j in 1:length(dayfac)){
		...
		cnames = c('SWup_avg','SWdn_avg','LWup_avg','LWdn_avg','PAR_avg','Tair_min','Tair_avg','Tair_max','RH_min','RH_avg','RH_max','WS_avg','Tsoil_min','Tsoil_avg','Tsoil_max','SM_avg','RF_avg','vpd')
		...
		for(k in 1:length(yID)){
			...
			#Adding VPD
			t = as.matrix(tempdat[which(substr(tempdat$yyyy.mm.dd.hh.mm,1,4) == yID[k]),which(substr(colnames(tempdat),1,4) == 'Tair')])
			vpd = .611*10^((7.5*t)/(t+237.3))
			time = tempdat[which(substr(tempdat$yyyy.mm.dd.hh.mm,1,4) == yID[k]),'yyyy.mm.dd.hh.mm']
			timeID = which(as.numeric(substr(time,12,13)) >= 6 & as.numeric(substr(time,12,13)) < 18)
			tab[k,'vpd']  = mean(vpd[timeID],na.rm=T)
		}
...
#Writing her out!
WriteXLS(x = 'daylist',ExcelFileName = paste(destdir,'Month_by_year_means_version_20141210_w_vpd.xls',sep=''),SheetNames = names(daylist),AdjWidth = T)
