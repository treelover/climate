ipif_plot = function(dlist,now,dayback,threeback){
...
#IPIF climate station. As of Oct 15th, 2014, the following instruments are installed and will be plotted:
# HMP45C (2)
#   - Temperature
#   - Relative Humidity
# CMP3, NR-Lite, LI-190 (LI is discontinued but still logging)
#   - SW up, Rnet, PAR
# CS700
#   - Rainfall
# RM Young 05106
...
mtext('IPIF',cex = 4, font = 2, outer = F,line = -10)
...
#A bit redundant here, but it's how it was written....
rdat = dlist$dat
colnames(rdat) = dlist$header2
	
R1 = rdat$SWup_Avg[which(dlist$time == threeback):which(dlist$time == now)]
R2 = rdat$PAR_Avg[which(dlist$time == threeback):which(dlist$time == now)]
R3 = rdat$Rnet_Avg[which(dlist$time == threeback):which(dlist$time == now)]
	
	#Fourth test: if the last 3 days are all NA
	#...............................................#
	if(all(is.na(R1)) | all(is.na(R2)) | all(is.na(R3))){
		plot(x = 1:10,y = 1:10, type = 'n',xlab = '',ylab='')
		text(x = c(5,5), y = c(5,4), labels = c('SWup, PAR or Rnet all NA.', 'Check data file!'))
	}else{
	#...............................................#
...
legend(x = 'topleft',fill = c('blue','red','forestgreen'),legend = c('SW up (CMP3)','PAR (LI-190)','Rnet (NR-Lite)'),bg='white')
}
#=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+#
#=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+#
#=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+#
