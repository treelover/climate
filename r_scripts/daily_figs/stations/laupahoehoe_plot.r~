laupahoehoe_plot = function(dlist,soil,now,dayback,threeback){
...
#Laupahoehoe climate station. As of Oct 15th, 2014, the following instruments are installed and will be plotted:
# HMP45C (2)
#   - Air Temperature
#   - Relative Humidity
# NR01
...
mtext('Laupahoehoe',cex = 4, font = 2, outer = F,line = -10)
	...
	if(length(dlist$time) < (6*24*3)){
		...
		text(x = c(5,5), y = c(5,4), labels = c('Laupahoehoe','Less than 3 days of data. Check data file!'))
		stop()
	}
	#...............................................#
	...
	if(length(which(dlist$time == threeback)) == 0 | length(which(dlist$time == now)) == 0){
		par(mfrow = c(1,1),mar = c(0,5,5,4),oma=c(3,0,0,0))
		plot(x = 1:10,y = 1:10, type = 'n')
		text(x = c(5,5), y = c(5,4), labels = c('Laupahoehoe','Last 3 days of data not found. Check data file!'))
		stop()
	}
	#...............................................#
...
R5 = rdat$BF3_PAR_Tot_Avg[which(dlist$time == threeback):which(dlist$time == now)]
	
	#Fourth test: if the last 3 days are all NA
	#...............................................#
	if(all(is.na(R1)) | all(is.na(R5))){
		plot(x = 1:10,y = 1:10, type = 'n',xlab = '',ylab='')
		text(x = c(5,5), y = c(5,4), labels = c('SWup or BF3 PAR all NA.', 'Check data file!'))
	}else{
	#...............................................#
...
col = 'blue',
xlab = '',
ylab = 'Radiation (W/m^2 or umol/s/m^2)',
ylim = c(min(c(R1,R5),na.rm=T),max(c(R1,R5),na.rm=T)),
las = 1)
...
lines(x = days, y = R5, lwd = 3, col = 'orange')

legend(x = 'topleft',fill = c('blue','orange'),legend = c('SW up','BF3 PAR'),bg='white')
	}
