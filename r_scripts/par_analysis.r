...
#FUNCTION - ploop
#---------------------------------------#
ploop = function(x,y,z){

	ncol = length(unique(z))
	cols = rainbow(ncol)
	...
	for(i in 1:ncol){
		...
		points(x = x[pID],y = y[pID], col = cols[i],pch = 8)
		}
		
		legend(x = 'topleft',fill = cols, title = 'Months since cal',
		legend = paste(((1:(ncol-1))-1),' - ',(1:(ncol-1)),sep = ''),bg = 'white',cex = 0.8)
}
#---------------------------------------#

#FUNCTION - regloop
#---------------------------------------#
regloop = function(x,y,z){
	...
	for(i in 1:ncol){
		...
		abline(reg, lwd = 2, col = cols[i])
		
		#rsq = c(rsq,round(summary(reg)$r.squared,2))
		}
...
#Start with Hakalau
#---------------------------------------#
hak[which(hak[,'SWup'] == -9999),'SWup'] = NA
hak[which(hak[,'PAR'] == -9999),'PAR'] = NA

time = as.POSIXct(hak[,'yyyy.mm.dd.hh.mm'])
...
jpeg(filename = paste(figdir,'hak_par.jpeg',sep=''), quality = 100, width = 1200,height = 500)
par(mfrow = c(1,2))
	...
	plot(x = pdatx,
	     ...
	     #ylim = c(1500,2500),
	     type = 'n',
	     main = 'Hakalau - from PAR cal date 2011-12-12')
